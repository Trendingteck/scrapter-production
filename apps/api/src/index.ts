...
app.post('/auth/signup', async (c) => {
  ...
  return c.json({ message: 'User created. Please check your email for verification.' });
});
...
app.post('/v1/chat', async (c) => {
  ...
  if (!isAdmin && !isPro) {
    ...
    if (usageCount > 100) {
      return c.json({ error: 'Daily limit reached for Free plan. Please upgrade.' }, 402);
    }
  }

  // Handle different providers
  if (provider === 'gemini' || (model && (model.includes('gemini') || model.startsWith('scrapter-')))) {
